        ...
        <thead>
            <tr role="row">
                <th class="sorting_desc" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1"
                    ...
                    aria-label="Order ID: activate to sort column ascending">Contributor ID</th>
            ...
            <tr class="filter">
                <th><input type="number" name="id" wire:model="contributor_id"
                ...
                <th style=""><select class="form-control form-control-sm form-filter datatable-input"
                        ...
                        @foreach (\App\Models\Contributor::STATUS as $status)
        ...
        <tbody>
            ...
            @foreach ($contributors as $contributor)
                <tr role="row" class="odd">
                    <td class="sorting_1">{{ $contributor->id }}</td>
                    @if (!is_null($contributor->img))
                        <td><img src="{{ Storage::url($contributor->img) }}" alt="Contributor Photo" id="contributor-img">
                        </td>
                    @else
                        <td>-</td>
                    @endif
                    <td>{{ $contributor->fname . ' ' . $contributor->lname }}</td>
                    <td>{{ $contributor->email }}</td>
                    <td>{{ $contributor->created_at->diffForHumans() }}</td>
                    <td>{{ $contributor->updated_at->diffForHumans() }}</td>
                    <td style=""><span class="{{ $contributor->contributor_status }}">{{ ucfirst($contributor->status) }}</span>
                    </td>
                    <td nowrap="nowrap" style="">
                        <div class="dropdown dropdown-inline"> <a href="javascript:;"
                        ...
                        </div> <a href="{{ route('contributors.edit', Crypt::encrypt($contributor->id)) }}"
                            ...
                            onclick="confirmDestroy('{{ Crypt::encrypt($contributor->id) }}', this)"
